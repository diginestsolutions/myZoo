...
const OrderCard = ({label, orderDate, mt, item, orderDetails}) => {
    ...
    const MakeActiveTrackItem = (item) => {
        ...
        //     payload: item
        // })
        navigation.navigate('TrackItem', { item: item });
    }
    ...
    const MakeActiveOrder = (item) => {
        ...
        navigation.navigate('OrderDetails', { productId: item?.products?.productId, orderId: item?._id, item: item })
    }
    ...
    <>
        ...
        >
            ...
            <Box w={'80%'}>
                ...
                {item?.orderDate && <Text fontWeight={400} fontFamily="body" fontSize={14} color='#008ECC' ml={2}>{moment(item?.orderDate).format("DD-MM-YYYY") }</Text>}
            </Box>
